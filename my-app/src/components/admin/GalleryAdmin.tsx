...
export default function GalleryAdmin() {
  ...
  return (
    <div className="max-w-md mx-auto p-4 text-gray-800">
      <h2 className="text-xl font-bold mb-4 text-center">📷 Upload Reference Images</h2>

      {/* Upload Field */}
      <label
        onDrop={handleDrop}
        onDragOver={(e) => e.preventDefault()}
        className="w-full border-2 border-dashed border-gray-300 hover:border-blue-500 rounded-xl py-10 px-4 flex flex-col items-center justify-center bg-gray-50 hover:bg-blue-50 transition"
      >
        <input
          type="file"
          accept="image/*"
          ...
          className="hidden"
        />
        <p className="text-sm font-medium text-gray-600">Tap or drag images to upload</p>
      ...
      <button
        ...
        className="w-full mt-4 px-6 py-2 bg-blue-600 hover:bg-blue-700 transition rounded-lg text-white font-medium shadow text-sm"
      >
      ...
      {uploadedImages.length > 0 && (
        <div className="mt-10">
          <h3 className="text-base font-semibold mb-3 text-center">🖼️ Your Gallery</h3>
          <div className="grid grid-cols-2 gap-4">
            {uploadedImages.map((img) => (
              <div key={img.id} className="relative group border rounded-xl overflow-hidden">
                <img src={img.url} alt="Uploaded" className="w-full h-36 object-cover" />
                <button
                ...
                <div className="absolute bottom-2 left-2 bg-white/90 px-1 rounded">
                  <select
